...
/*
Purpose:          Controller class for SearchInfoChairPage
Collaboration:    Created by ApplicantInfoChairPage and FacultySearchesRegularPage
Author:           Jacob Wolfe
 */

public class SearchInfoChairController extends UIPageController {
    ...
    /*
    Function:       Creates a new SearchInfoChairController
    Parameters:     InformationPackager packager - contains account and search information
    Preconditions:  SearchInfoChair page has been requested
     */
    public SearchInfoChairController(InformationPackager packager) {
        this.account = packager.getAccount();
        this.search = packager.getSearch();
    }

    /*
    Function:       Initializes the Search Info Chair Page
    Parameters:     none
     ...
     */
    public void openApplicantInfoChairPage(MouseEvent event) {
        ...
        if (event.getButton().equals(MouseButton.PRIMARY) && event.getClickCount() == 2 && event.getTarget().getClass() != TableColumnHeader.class) {
            ...
            if (applicant != null) {
                ...
                try {
                    FXMLLoader loader = new FXMLLoader((getClass().getResource("UIPagesAndForms/ApplicantInfoChairPage.fxml")));
                    ApplicantInfoChairController controller = new ApplicantInfoChairController(packager);
                    loader.setController(controller);
                    Parent parent = loader.load();

                    Scene scene = new Scene(parent);
                    stage.setScene(scene);
                    stage.setTitle("Applicant Information - " + applicant.getFirstName() + " " + applicant.getLastName());
                    stage.show();
                } catch (IOException e) {
                    System.out.println(e.getMessage());
                }
